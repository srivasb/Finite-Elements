...
% El objetivo del código es encontrar los coeficientes de la combinación 
% lineal que minimizan el residuo de la aproximación. 
%
...
fi=@(x) exp(-x)*x^(1/3); % Se define la función que se desea aproximar
Nm=@(x,m) x^m;           % Se define la familia de funciones base a emplear
M=10;                    % Se define la cantidad de términos a emplear en la aproximación
...
for i=1:M
  for j=1:M
   %Llenado de la matriz K
    K(i,j)=int(Nm(x,i)*Nm(x,j),x,0,1);      
  end
  %Llenado del vector F
  F(i)=int(fi(x)*Nm(x,i),x,0,1);  
end
...
X=linspace(0,1,np)';  %Conjunto de valores x en los que se evalúan las funciones
